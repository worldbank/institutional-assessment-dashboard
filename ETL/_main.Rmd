...
- Inputs:
  ...
  - `data/output/region_list_compliment.rds`, complimentary for groups in the case of splitting SSA into AFW and AFE
...
coverage_regional |>
  ...
  geom_vline(xintercept = 0, linetype = "solid", linewidth = 0.5, color = "grey3") + 
  geom_vline(xintercept = 75, linetype = "dotted", linewidth = 1.5, color = "grey") +
  geom_vline(xintercept = 100, linetype = "solid", linewidth = 0.5, color = "grey3") +
  ...
  geom_jitter(aes(x = group_av, y = 0), 
              ...
              height = 0,  
              alpha = 0.5) +  
ggtitle("Global Data Landscape: Gaps in Country-Level Average Coverage by Region",
        subtitle = "Insights from the Latest CLIAR Data: 2019-2023") +
  labs(x = "Average Coverage (%)",
       y = "Density",
       fill = "Region",
       caption = "Source: CLIAR Dashboard. CLIAR is an internal dashboard that offers the most comprehensive suite of governance indicators, compiling more than 400 indicators\n across seven critical governance dimensions from over 30 renowned data sources, covering more than 180 countries over a 30-year period.") +
...
ggsave_full(
  here("figs", "region_missingness.png")
)
...
coverage_income |>
  ...
  geom_jitter(aes(x = group_av, y = 0), 
              ...
              height = 0,  
              alpha = 0.5) +  
  ggtitle("Uneven Data Landscape: Gaps in Country-Level Average Coverage by Income Group",
    subtitle = "Insights from the Latest CLIAR Data: 2019-2023"
   ) + 
  labs(x = "Average Coverage (%)",
       y = "Density",
...
ggsave_full(
  here("figs", "income_missingness.png")
)
...
coverage_family |>
  ...
  geom_jitter(aes(x = group_av, y = 0), 
             ...
             height = 0, 
             alpha = 0.5) +
  ggtitle("Uneven Data Landscape: Gaps in Country-Level Average Coverage by Cluster",
          subtitle = "Insights from the Latest CLIAR Data: 2019-2023") + 
  labs(x = "Average Coverage (%)", 
       y = "Density", 
       fill = "Family Name", 
       caption = "Source: CLIAR Dashboard. CLIAR is an internal dashboard that offers the most comprehensive suite of governance indicators, compiling more than 400 indicators\n across seven critical governance dimensions from over 30 renowned data sources, covering more than 180 countries over a 30-year period.") + 
  facet_wrap(~ wrap_facet_titles(family_name, width = 30)) + 
  scale_fill_brewer(palette = "Paired") +
  coord_cartesian(ylim = c(0, 1)) +
  scale_x_continuous(breaks = c(0, 25, 50, 75, 100)) +
  coord_flip()
...
ggsave_full(
  here("figs", "clusters_missingness.png")
)
```
...
coverage_regional |> 
  ...
  ggplot(aes(x = region_short,
   ...
   geom_label_repel(aes(label = country_code, y = group_av), 
                    ...
                    direction = "both", 
                    hjust = 0.5) +  
    ggtitle("Uneven Data Landscape: Gaps in Country-Level Coverage within Regions",
    subtitle = "Focus: CTF CLIAR Indicators (2019-2023)") +
   labs(
     x = "Region",
     y = "Average Values",
     shape = "Values",
...
ggsave_full(
  here("figs", "within_region_missingness.png")
)
```
...
coverage_income |> 
  ...
  geom_segment(aes(x = income_group,
   ...
   geom_label_repel(aes(label = country_code, y = group_av), 
                    ...
                    direction = "both", 
                    hjust = 0.5) +  
  ggtitle("Uneven Data Landscape: Gaps in Country-Level Coverage within Income Groups",
    subtitle = "Focus: CTF CLIAR Indicators (2019-2023)") +
  labs(
    x = "Region",
    y = "Average Values",
...
ggsave_full(
  here("figs", "within_income_missingness.png")
)
...
consult Apendix (a) of this document to gain more insight into the update cycles."
    ) +
    scale_fill_manual(
...
```{r}
### Data Preparation
ctf_dynamic_share <- ctf_dynamic %>%
  group_by(year, family_name) %>%
  summarise(
    total_indicators = n(),
    available_indicators = sum(!is.na(indicator_value))
  ) %>%
  mutate(global_coverage = (available_indicators / total_indicators) * 100) %>%
  ungroup()


### Only country level
ctf_dynamic_share_cc <- ctf_dynamic %>%
  group_by(year, family_name, country_code ) %>%
  summarise(
    total_indicators = n(),
    available_indicators = sum(!is.na(indicator_value))
  ) %>%
...
### Plot
ctf_dynamic_share |>
  filter(family_name %in% selected_family_names) %>% 
  ggplot(aes(x = year, y = global_coverage, color = as.factor(family_name))) +
    geom_hline(yintercept = 0, linetype = "solid", linewidth = 0.5, color = "grey3") + 
  geom_hline(yintercept = 50, linetype = "dotted", linewidth = 1, color = "grey") +
  geom_hline(yintercept = 100, linetype = "solid", linewidth = 0.5, color = "grey3") +
  geom_rect(aes(xmin = -Inf, xmax = Inf, ymin = 50, ymax = 100),
  ...
  annotate("text",
           x = Inf,
           y = Inf,
           label = "Coverage Gap Zone",
           hjust = 1.1,
           vjust = 3,
           color = "white",
           size = 4) +
 ggtitle("Uneven Data Landscape: Coverage Gaps",
    subtitle = "Insights from the Latest CLIAR Database: 2019-2023"
   ) + 
  labs(
    x = "Year",
    y = "Indicator-Country Coverage (%)",
    color = "Key Dimensions",
    caption = "Source: World Bank CLIAR Database."
  ) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(size = 12, hjust = .5),
...
ggsave(
  here("figs", "dynamic_clusters_missingness_zone_.png")
)
...
<!--chapter:end:08-coverage-report.Rmd-->

---
output: github_document
---

# CLIAR REPRODUCIBILITY PACKAGE

This shows how to reproduce the CLIAR dataset which is used in implementing the dashboard. You simply need to run this RMarkdown file to reproduce within the project environment to reproduce the data. 

Here are the steps: 

1) Click the `code.Rproj` within this ETL folder to activate the project environment. 

2) Once in the environment, simply run the README.Rmd file in this ETL folder's base directory.
...
bookdown::render_book(input = here::here("."))
