...
plot_country_theme <- theme(
    ...
    strip.background = element_rect(fill=NA),
    strip.text = element_text(size = ifelse(params$region=="Europe & Central Asia",11,14), color = id_colors$primary),
...
data_plot <- 
  data |> 
  distinct(country_name, ecb, country_code) |>
...
ggplot(data = data_plot) +
  ...
  geom_text(
    aes(
      x = column,
      y = -row - 0.325,
      label = str_wrap(paste0(country_name, " (",country_code,")"),22),
    ),
    size = 2.25,
    vjust = 1,
    hjust = 0.5,
...
ggplot() +
  ...
  geom_point(
    data = data_plot,
    aes(
      y = fct_rev(factor(var_name, levels = family_order)),
      x = rank,
  ...
  geom_text(
    ...
    size = ifelse(params$region=="Europe & Central Asia",1.85,2.5),
    color = "black",
    vjust = 0.5,
    fontface = "bold"
  ) +
...
data_avg <- data_plot |>
  ...
  mutate(
    ...
    status = case_when(
      dtt <= cutoff[1]/100 ~ paste0("Weak (bottom ", cutoff[1],"%)"),
      dtt > cutoff[1]/100 & dtt <= cutoff[2]/100 ~ paste0("Emerging (",cutoff[1],"% - ",cutoff[2],"%)"),
        dtt > cutoff[2]/100 ~ paste0("Strong (top ",100-cutoff[2],"%)")
      ),
      nrank = min_rank(-value)
  ) |>
  rename(dtf = value) |>
  mutate(var_name = paste0(family, " Average"))
