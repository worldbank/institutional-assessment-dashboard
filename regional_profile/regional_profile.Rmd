...
ggplot() +
  ...
  theme(
    ...
    legend.title = element_text(size=9, margin = margin(t = 0, r = 0, b = 0, l = 0)),
    ...
    axis.text.x = element_text(size = 8, color = id_colors$primary, margin = margin(t = 20, r = 0, b = 5, l = 0)),
    legend.box = "vertical"
  )
...
```{r clusters_radar, fig.cap="Digital and Data Institutions static CTF", fig.height = 7.5, fig.width = 9, warning=F}

data_plot <- bind_data |>
  filter(comparison_group == "Latin America & Caribbean") |>
...
ggplot(
  data_plot |> filter(country_name == "Brazil") |> filter(!grepl("Average", var_name)),
  aes(
  ...
  geom_hline(
    aes(yintercept = y), 
    data.frame(y = c(0.25,0.5,0.75,1)),
    color = id_colors$primary,
    linewidth = 0.15,
    linetype = 1
  ) + 
  geom_bar(stat = "identity", color = "white", width = 0.8, alpha = 0.85, position = position_stack()) +
  geom_text(
    family = "Roboto Condensed", color = id_colors$primary, size = 3.5, nudge_y = 0.15,
    stat="identity"
  ) +
  #geom_vline(
  #  xintercept = 1:6 - 0.5, 
  ...
  #scale_fill_stepsn(
  #  "Índice de Prosperidade das Cidades",
  #  limits = c(0,100),
  #  breaks = c(40,50,60,70,80),
  #  values = scales::rescale(c(0,40,50,60,70,80,100)),
  #  colours = cores_cpi
  #) +
  facet_wrap(~country_name, ncol = 3) +
  #guides(
  #  fill = guide_colorsteps(
  #    barwidth = 15, barheight = .4, title.position = "top", title.hjust = .5, 
  #    even.steps = FALSE,
  #    show.limits = T
  #  )
  #) +
  labs(
    ...
    strip.background = element_rect(fill=NA),
    strip.text = element_text(size = 12, color = id_colors$primary, face = "bold"),
    
    axis.ticks = element_blank(),
    axis.title = element_blank(),
    axis.text.y = element_blank(),
    axis.text.x = element_text(size = 10, color = id_colors$primary, margin = margin(t = 0, r = 0, b = 0, l = 0)),
    legend.box = "vertical"
  )
