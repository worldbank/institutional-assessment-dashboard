...
## 6. Stress Tests for Coverage

```{r}
# 2.	We exclude variables that over the past 5 years have data on fewer than 100 countries (we can debate whether 100 is the right number but we need a number). This is the data cover at least once over the past 5 years (2018-2022) each of the 100 countries.

# this line generates a table where each row is
# a country-variable combination,
# and the attributes are the number and percentage
# of missingness for that combination
country_coverage_last_5_years <- full_data |> 
  filter(
    between(as.numeric(year), 2016, 2022)
  ) |>   
  select(
    c(country_name, any_of(vars_all))
  ) |> 
  pivot_longer(
    cols = c(any_of(vars_all)),
    names_to = "variable"
  ) |> 
  filter(!is.na(value)) |> 
  group_by(variable) |>
  summarise(
    country_coverage = n_distinct(country_name)
  ) |> 
  # avoid excluding pefa variables
  filter(!str_detect(variable, "pefa")) |> 
  # fix variable names
  pivot_wider(names_from = variable, values_from = country_coverage) |> 
  rename_from_dictionary(db_variables) |> 
  pivot_longer(
    cols = c(everything()),
    names_to = "variable",
    values_to = "country_coverage"
  )

country_coverage_last_5_years |>
  filter(country_coverage < 100) |> 
  ggplot(
    aes(country_coverage, reorder(variable, country_coverage))
  ) +
  geom_col(width = 0.5) +
  geom_vline(
    xintercept = 100,
  ...
  labs(
    x = "Countries Covered At Least Once in 2016-2022",
    y = "Variable",
    title = "Country Coverage by Variable: At Least Once from 2016-2022",
    caption = "Excludes PEFA variables"
  )
