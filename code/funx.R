...
calculate_coverage <- function(indicator, id) {                                        ### Change level of observation, from indicator to country_name
  coverage_id <- n_distinct({{id}}[!is.na(indicator)])                                 ### or add a second country_name_id as a parameter for the rest?
                                                                                      ### distinct -> use paste
  return(coverage_id)
}

# flag_continued <- function(indicator, year_id, ref_year){                            ### NO USE
...
# }
                                                                                       ### modify level
flag_minimum_coverage <- function(indicator, country_id, year_id){                     ### modify flag for printing year (vector?)
  # this function returns a flag for countries with less than minimum coverage:        ### not ifelse, but use paste('vector'), sep = "_", collapse = '?')
...
calculate_time_range <- function(indicator, time_id){                                 ### Keep and change from indicator to country level
  year_range <- paste0(
...
compute_coverage <- function(data, country_id, year_id, ref_year){                   ### Indicator level do group_by
  data_coverage <- data |>
