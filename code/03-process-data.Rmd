...
Building on the 'compute_coverage' function, this subsection makes an assessment on the  available at a country-level information to create a table that overvieews at range, latest year, missing years (vector), missing years share (%) and a logic benchmarked column.
...
### [MAKE THIS MORE EFICIENT, IFELSE TAKES FOREVER!]
cliar_global_coverage_renamed <- cliar_global_coverage |>
  # Extract available_share from a list-column and convert it to numeric if it's a percentage
  mutate(available_share = as.character(available_share)) |>  # Ensure it's a character vector
  mutate(available_share = ifelse(available_share != "Not available", 
                                  as.double(sub("%", "", available_share)) / 100, 
                                  NA_real_)) |> ### DID NOT WORK
  left_join(
    db_variables |> select(variable, var_name, source, family_name),
