...
```{r read_in}
efi <- read_dta(
  here("data", "input", "efi", "EFI360_CLIAR_1990-2022_032124.dta")
)
...
vdem <- read_csv(
  here("data", "input", "vdem", "2024", "vdem_19902023.csv.gz")
)
...
columns_to_drop <- c(
  ...
  "WDI_DCODATOTLGNZS",
  "WDI_DTDODPVLXGNZS",
  # "WDI_PERSIALLSIADQPOPTOT",
  # "WDI_PERALLSPADQPOPTOT",
  # "WDI_PERSAALLSAADQPOPTOT",
  # "WDI_PERSIALLSICOVPOPTOT",
  # "WDI_PERSIALLSICOVQ1TOT",
  # "WDI_PERALLSPCOVPOPTOT",
  # "WDI_PERSAALLSACOVQ1TOT",
  # "WDI_SHMEDCMHWP3",
  "WDI_SIPOVMDIM",
  "WDI_SIPOVMDIMXQ"
)

columns_to_drop <- tolower(columns_to_drop)
...
```{r wdi rename db_variables convention}
# Extract column names
wdi_clean_cols <- colnames(wdi_clean)
wdix_cols <- colnames(wdix_clean)

# Identify extra columns
extra_wdi_clean_cols <- setdiff(wdi_clean_cols, wdix_cols)
extra_wdix_cols <- setdiff(wdix_cols, wdi_clean_cols)

# Identify common columns to compare names
common_cols <- intersect(wdi_clean_cols, wdix_cols)


# Rename columns in wdi_clean and wdix to lower case for comparison
wdi_clean_renamed <- wdi_clean %>%
  rename_with(~ tolower(gsub(" ", "_", .)))

wdix_renamed <- wdix %>%
  rename_with(~ tolower(gsub(" ", "_", .)))

# Compare common columns in a case-insensitive manner
diff_names <- setdiff(tolower(common_cols), tolower(gsub(" ", "_", wdix_cols)))

# Print results
print("Extra columns in wdi_clean:")
print(extra_wdi_clean_cols)

print("Extra columns in wdix:")
print(extra_wdix_cols)

print("Differences in column names for common columns:")
print(diff_names)

###
# RESULTS
# > print(extra_wdi_clean_cols)
# [1] "wdi_persiallsiadqpoptot" "wdi_perallspadqpoptot"   "wdi_persaallsaadqpoptot" "wdi_persiallsicovpoptot"
# [5] "wdi_persiallsicovq1tot"  "wdi_perallspcovpoptot"   "wdi_persaallsacovq1tot"  "wdi_shmedcmhwp3"        
# > print("Extra columns in wdix:")
# [1] "Extra columns in wdix:"
# > print(extra_wdix_cols)
# [1] "wdi_seadtlitrfezs" "wdi_seadtlitrmazs" "wdi_seadtlitrzs"   "wdi_shdynmort"     "wdi_shstaanvczs"   "wdi_shstammrt"    
# [7] "wdi_spdynamrtfe"   "wdi_spdynamrtma" 
...
cliar_indicators_clean <- cliar_indicators |>
  ...
  filter(!country_code %in% c("DDR", "YMD")) # Filter 2 rows
```
...
# unmatched_rows <- cliar_indicators_clean %>%
#   distinct(country_code, country_name) %>%
#   anti_join(
#     wb_country_list %>%
#       distinct(country_code, country_name),
...
# print("Unmatched rows in cliar_indicators_clean:")
# print(unmatched_rows)
# # DDR & YMD 
# # GERMANY (EAST) DEMOCRATIC REPUBLIC should be DEU
# # YEMEN, SOUTH (ADEN) should be YEM
