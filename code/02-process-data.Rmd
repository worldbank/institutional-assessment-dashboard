...
```{r read_in}
cliar_metadata_id <- read_csv(
  here("data", "input", "cliar", "cliar_metadata_id.csv")
)  |> 
  clean_names() |> 
  mutate(
    cliar_id = make_clean_names(cliar_id)
  )
...
efi_clean <- efi |> 
  ...
  rename_with(
    \(x) str_replace(x, "wb_survey", "wb_enterprisesurveys_ic_frm"),
  ...
  map(
    ...
    ~ mutate(
        ...
        country_code = case_when(
          ...
          country_code == "PSG" ~ "PSE", # gaza and west bank
...
cliar_metadata_id_indicators <- cliar_metadata_id|> 
  filter(
    status != "Removed"
  ) |> 
  select(
...
cliar_metadata_id_indicators |> 
  anti_join(cliar_indicators_id, by = "cliar_id") |> 
  as.data.frame()
  
cliar_indicators_id |> 
  anti_join(cliar_metadata_id_indicators, by = "cliar_id") |> 
  as.data.frame()
...
cliar_indicators_clean <- cliar_indicators |> 
  select(
    ...
    country_code,
    year,
    any_of(cliar_metadata_id_indicators |> pull(cliar_id))
  )
...
cliar_indicators_diagnostic <- cliar_indicators_clean |> 
  # exclude index from computing coverage
  select(-index) |> 
  compute_coverage(country_code, year) |> 
  left_join(
    cliar_metadata_id |> select(cliar_id, indicator, source),
    by = c("Indicator" = "cliar_id")
  ) |> 
  select(
    `Indicator`,
