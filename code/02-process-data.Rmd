...
and clean them. In order to ensure clean column names and accurate variables, certain datasets are edited to create new variables or to change other factors. More detail given in each subsection
```{r clean_data}
...
## 5. Data Quality Control: verify country_code, country_name consistency and completeness of country-year combinations
This section ensures that the country_code code and the country name are
consistent, as well as Check this by performing an anti-join in both directions on the
