...
# dynamic
ctf_dynamic <- ctf_dynamic |> 
  compute_family_average(
    vars = vars_dynamic_family_ctf,
    type = "dynamic",
    db_variables = db_variables
  )

# separate pipeline for partial (every two-years indicator)
ctf_dynamic_partial <- ctf_dynamic |> 
  # only retain even years because data is updated every two-years
  filter(
    year %% 2 == 0
  ) |> 
  compute_family_average(
    vars = vars_dynamic_partial_ctf,
    type = "dynamic",
    db_variables = db_variables
  )
```
