...
# compute confidence intervals for each institutional family
# for the static ctf
ctf_confint_static <- ctf_complete |> 
  select(
    country_code,
    country_name,
    income_group,
  ...
  mutate(
    ...
    across(
      ends_with("avg"),
      ~ . + 1.96 * std_err(.),
      .names = "{.col}_conf_int_upper"
    ),
    across(
      ends_with("avg"),
      ~ . - 1.96 * std_err(.),
      .names = "{.col}_conf_int_lower"
    )
  ) %>%
  select(
...
# for the dynamic ctf
ctf_confint_dynamic <- ctf_dynamic_complete |> 
  filter(
    year %in% seq(2016, max(year), 2)
  ) |> 
  select(
    country_code,
    country_name,
