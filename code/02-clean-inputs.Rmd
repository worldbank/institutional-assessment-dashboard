# Cleaning files from 2024 data bash

-   Input: `data/input/**`
-   Output: `data/output/2024/clean-2024version-*.csv.gz`
 

This script uses the 'RStrata' package for initial cleaning of manually imported data,
streamlining the process and preparing key indicators.
...
library(countrycode)
library(purrr)
library(vdemdata) # R Package version: ?
library(WDI) # R Package version: ?
library(RStata) # Do files
```


## 1. Import data inputs

The data, initially imported manually, will be filtered
(relevant indicators only) and cleaned either calling .do files or in R. 
This step enables automation for future data update. Once cleaned, 
standardized CSV files will be exported to be further procesed.


### 1.1 Do files

```{r read_in, eval=FALSE, include=FALSE}
...
stata("/data/input/heritage/clean heritage.do")
...
wdi <- WDI(
  ...
  indicator = c(
    ...
    "IQ.CPA.TRAD.XQ", "IQ.CPA.TRAN.XQ", "per_si_allsi.adq_pop_tot",
    "per_allsp.adq_pop_tot", "per_sa_allsa.adq_pop_tot",
    "per_si_allsi.cov_pop_tot", "per_si_allsi.cov_q1_tot",
    "per_allsp.cov_pop_tot", "per_sa_allsa.cov_q1_tot",
...
vdem_clean <- vdem |>
  ...
  select(
    ...
    v2x_civlib,
    v2x_cspart,
    # v2cl_acjst, ### Review this variable
    v2clacjstm,
    v2clacjstw,
    v2lgqugen,
    v2lgfemleg,
