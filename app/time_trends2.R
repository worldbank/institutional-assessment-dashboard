...
ggsave("figs/trends_overview.png", bg = "white", width = 12, height = 8, scale = 0.8)
...
gbid_jordan %>% 
  filter(family == "") %>%
  filter(indicatornamefromdefinitions != "Labor market") %>%
  mutate(
    diff = if_else(dtf_2017_2020 > dtf_2014_2016, "Improved", "Worsened"),
    indicatornamefromdefinitions = fct_reorder(indicatornamefromdefinitions, -dtf_2017_2020)
  ) %>% 
  ggplot(
    aes(
  ...
  geom_segment(
    aes(
      x = dtf_2014_2016,
      xend = dtf_2017_2020,
      yend = indicatornamefromdefinitions,
      color = diff
    ),
    arrow = arrow(length = unit(0.3, "cm")),
    size = 1.5
