...
vars_original <-  db_variables %>%
  filter(select==1 & var_level == "indicator" & data_source=="original") %>%
  .$variable

# Variables from Gov360 API ======================
vars_api <-  db_variables %>%
  filter(select==1 & var_level == "indicator" & data_source=="api") %>%
  .$variable

# Variables from Additions source  ======================
vars_additions <-  db_variables %>%
  filter(select==1 & var_level == "indicator" & data_source=="additions") %>%
  .$variable
